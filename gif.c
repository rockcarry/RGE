...
static int _gif_fio_getc(void *fp)
{
    ...
    if (context->gifmode)
    {
        ...
        {
            context->counter = context->basedrv->getc(context->basefp);
            if (context->counter == EOF || context->counter == 0) return EOF;
        }
        context->counter--;
    }
    return context->basedrv->getc(context->basefp);
}
...
static int _gif_fio_putc(int c, void *fp)
{
    ...
    {
        if (context->counter == 0) {
            if (EOF == context->basedrv->putc(255, context->basefp)) return EOF;
        }
        if (++context->counter == 255) context->counter = 0;
    }
    return context->basedrv->putc(c, context->basefp);
}

static size_t _gif_fio_read(void *buf, size_t size, size_t count, void *fp)
{
    GIF_FIO_CONTEXT *context = (GIF_FIO_CONTEXT*)fp;
    return context->basedrv->read(buf, size, count, context->basefp);
}

static size_t _gif_fio_write(const void *buf, size_t size, size_t count, void *fp)
{
    GIF_FIO_CONTEXT *context = (GIF_FIO_CONTEXT*)fp;
    return context->basedrv->write(buf, size, count, context->basefp);
}

static long _gif_fio_tell(void *fp)
{
    GIF_FIO_CONTEXT *context = (GIF_FIO_CONTEXT*)fp;
    return context->basedrv->tell(context->basefp);
}

static int _gif_fio_seek(void *fp, long offset, int org)
{
    GIF_FIO_CONTEXT *context = (GIF_FIO_CONTEXT*)fp;
    return context->basedrv->seek(context->basefp, offset, org);
}

static int _gif_fio_eof(void *fp)
{
    GIF_FIO_CONTEXT *context = (GIF_FIO_CONTEXT*)fp;
    return context->basedrv->eof(context->basefp);
}
...
static int _gif_fio_flush(void *fp)
{
    ...
    {
        if (context->counter != 0) {
            if (EOF == context->basedrv->seek(context->basefp, -context->counter-1, SEEK_CUR)) return EOF;
            if (EOF == context->basedrv->putc(context->counter, context->basefp)) return EOF;
            if (EOF == context->basedrv->seek(context->basefp,  context->counter,   SEEK_CUR)) return EOF;
        }
        if (EOF == context->basedrv->putc(0, context->basefp)) return EOF;
    }
    return context->basedrv->flush(context->basefp);
}

static FIODRV GIF_FIO_DRV =
{
    _gif_fio_open,
    _gif_fio_close,
    _gif_fio_getc,
...
void* gifdecodeinit(void *fp, FIODRV *drv)
{
    ...
    // init fp of context
    context->fp = GIF_FIO_DRV.open((const char*)fp, (const char*)drv);
    if (!context->fp) {
        failed = TRUE;
        goto done;
    }
    ...
    /* read gif header */
    GIF_FIO_DRV.read(&gifheader, sizeof(gifheader), 1, context->fp);
    ...
    GIF_FIO_DRV.read(&logscrdesc, sizeof(logscrdesc), 1, context->fp);
    ...
    if (logscrdesc.global_color_table_flag == 1)
    {
        ...
        GIF_FIO_DRV.read(palbuf, context->gpalsize * 3, 1, context->fp);
    ...
    context->dataoffset = GIF_FIO_DRV.tell(context->fp);
...
BOOL gifdecodeframe(void *ctxt, int *type, BMP *pb, int *xpos, int *ypos, GIF_CTRL *ctrl, GIF_TEXT *text)
{
    ...
    *type   = GIF_FRAME_TYPE_UNKNOWN;
    curbyte = GIF_FIO_DRV.getc(context->fp);
    switch (curbyte)
    {
    case GIF_IMG_SPRT:
        /* set type */
        *type = GIF_FRAME_TYPE_IMAGE;
        ...
        GIF_FIO_DRV.read(&imagedesc, sizeof(IMAGEDESCRIPTOR), 1, context->fp);
        ...
        *ypos = imagedesc.image_top_position;
        ...
        if (imagedesc.local_color_table_flag)
        {
            ...
            GIF_FIO_DRV.read(palbuffer, palsize * 3, 1, context->fp);
            for (i=0; i<palsize; i++)
            {
        ...
        codesize = GIF_FIO_DRV.getc(context->fp);
        ...
        memctxt = MEM_FIO_DRV.open((const char*)pb->pdata, (const char*)(pb->stride * pb->height));
        giffio_setgifmode(context->fp, TRUE);
        alignbyte(context->fp, &GIF_FIO_DRV);
        lzwdecode(&context->lzwcodec, memctxt, &MEM_FIO_DRV, context->fp, &GIF_FIO_DRV);
        giffio_setgifmode(context->fp, FALSE);
        MEM_FIO_DRV.close(memctxt);
    ...
    case GIF_EXT_INTR:
        ...
        case GIF_CTRL_EXT:
            ...
            GIF_FIO_DRV.read(&gctrlext, sizeof(GRAPHCTRLEXT), 1, context->fp);
        ...
        case GIF_TEXT_EXT:
            ...
            GIF_FIO_DRV.read(&gtextext, sizeof(PLAINTEXTEXT), 1, context->fp);
            curbyte = GIF_FIO_DRV.getc(context->fp);
            ...
            *ypos = gtextext.text_grid_top_position;
            ...
            curbyte = GIF_FIO_DRV.getc(context->fp);
            if (curbyte == 0) break;
            GIF_FIO_DRV.read(text->text, curbyte, 1, context->fp);

            for (;;)
            {
                curbyte = GIF_FIO_DRV.getc(context->fp);
                if (curbyte == 0) break;
                GIF_FIO_DRV.seek(context->fp, curbyte, SEEK_CUR);
            }
            /* -- we only support 256 bytes text length -- */
            break;
...
//          log_printf("unsupported gif ext_intr !\r\n");
    ...
    case ';':
        GIF_FIO_DRV.seek(context->fp, context->dataoffset, SEEK_SET);
...
void* gifencodeinit(void *fp, FIODRV *drv, int width, int height, BYTE bkcolor, BYTE *pal, int size)
{
    ...
    /* write gif header */
    GIF_FIO_DRV.write(&gifheader, sizeof(gifheader), 1, context->fp);
    ...
    GIF_FIO_DRV.write(&logscrdesc, sizeof(logscrdesc), 1, context->fp);
    ...
    if (context->gpalsize > 0) {
        ...
        GIF_FIO_DRV.write(palbuf, 256 * 3, 1, context->fp);
    }
...
BOOL gifencodeframe(void *ctxt, int type, BMP *pb, int xpos, int ypos, GIF_CTRL *ctrl, GIF_TEXT *text)
{
    ...
    case GIF_FRAME_TYPE_IMAGE:
        ...
        GIF_FIO_DRV.putc(GIF_IMG_SPRT, context->fp);
        ...
        GIF_FIO_DRV.write(&imagedesc, sizeof(imagedesc), 1, context->fp);
        ...
        memctxt = MEM_FIO_DRV.open((const char*)pb->pdata, (const char*)(pb->stride * pb->height));
        giffio_setgifmode(context->fp, TRUE);
        lzwencode(&context->lzwcodec, context->fp, &GIF_FIO_DRV, memctxt, &MEM_FIO_DRV);
        GIF_FIO_DRV.flush(context->fp);
        giffio_setgifmode(context->fp, FALSE);
        MEM_FIO_DRV.close(memctxt);
        break;
    ...
    case GIF_FRAME_TYPE_CTRL:
        ...
        GIF_FIO_DRV.putc(GIF_EXT_INTR, context->fp);
        GIF_FIO_DRV.putc(GIF_CTRL_EXT, context->fp);

        /* write block size to file */
        GIF_FIO_DRV.putc(sizeof(gifctrlext), context->fp);
        ...
        GIF_FIO_DRV.write(&gifctrlext, sizeof(gifctrlext), 1, context->fp);

        /* write block terminator to file */
        GIF_FIO_DRV.putc(0, context->fp);
        break;

    case GIF_FRAME_TYPE_END:
        /* write the gif file Trailer */
        GIF_FIO_DRV.putc(';', context->fp);
        break;
    ...
    case WM_TIMER:
        while (1) {
            ...
            case GIF_FRAME_TYPE_IMAGE:
                ...
                log_printf("\r\n");
                */
                putbmp(&SCREEN, xpos, ypos, &g_mybmp);
                break;
            case GIF_FRAME_TYPE_CTRL:
                /*
